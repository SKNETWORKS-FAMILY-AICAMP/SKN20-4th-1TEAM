...
{{
    ...
    def route(self, query: str) -> dict:
        ...
        try:
            ...
            print(f"[QueryRouter] JSON 파싱 에러. 원본: {result_str}")
            return {
                "is_valid": True,
...
{query}""")
    ...
    def query(self, user_query: str) -> Dict:
        ...
        print(f"[디버그] 감지된 지역 정보: {current_region_info}")
        ...
        print(f"[디버그] Router 결과: {route_result}")
        ...
        print(f"[디버그] 정제된 키워드: {refined_keyword}")
        ...
        print(f"[디버그] 생성된 멀티쿼리: {all_queries}")
        ...
        if not active_region_info.get('is_national') and (active_region_info.get('sigungu_code') or active_region_info.get('sido_code')):
            ...
            print(f"[디버그] 지역 코드 감지됨: {detected_region_code}")
        
        print(f"[디버그] 생성된 필터: {db_filter}")

        # [Step 4 수정] Retrieval: 앙상블 검색 (멀티쿼리 리스트 전달)
        search_results = self.retriever.retrieve(all_queries, db_filter)
        print(f"[디버그] 검색 결과 (Dense): {len(search_results['dense'])}, BM25: {len(search_results['bm25'])}")

        # [Step 5] RRF & Boosting
        top_docs = self.rrf.fuse(search_results, top_k=20)
        print(f"[디버그] RRF 후 문서 수: {len(top_docs)}")

        # [Step 6 수정] 문서가 없으면 필터 없이 멀티쿼리로 재검색
        if not top_docs:
            print("[디버그] 필터링된 문서 없음. 필터 제거 후 재검색...")
            search_results = self.retriever.retrieve(all_queries, None)
            print(f"[디버그] 필터 제거 후 검색 결과 (Dense): {len(search_results['dense'])}, BM25: {len(search_results['bm25'])}")
            top_docs = self.rrf.fuse(search_results, top_k=20)
            print(f"[디버그] 필터 제거 후 RRF 결과: {len(top_docs)}")

        # [Step 7] Post-Filtering: Python 레벨의 강화된 지역 필터링
        final_docs = self.region_filter.post_filter(top_docs, detected_region_code)
        print(f"[디버그] Post-filter 후 최종 문서 수: {len(final_docs)}")
        ...
        if detected_region_code and final_docs:
            ...
            for doc in final_docs:
                ...
                if detected_region_code in doc_sido or detected_region_code in doc_sigungu:
                    ...
                    print(f"[디버그] 필터 제외됨 - 정책: {doc.metadata.get('정책명')}, 시도: {doc_sido}, 시군구: {doc_sigungu}")
            
            final_docs = region_validated_docs

        # [Step 8] Answer Generation
        context = self.memory.get_context()
        docs_text = "\n\n".join([f"[{d.metadata.get('정책명')}]\n{d.page_content[:400]}" for d in final_docs[:3]])
...
def initialize_test_system():
    ...
    if not all_docs_data or not all_docs_data.get('documents'):
        ...
        print(f"Metadata: {all_docs_data}")
        raise ValueError("VectorDB에서 문서를 찾을 수 없습니다.")
    
    print(f"로드된 문서 수: {len(all_docs_data['documents'])}")
...
def main():
    ...
    print("🌟 청년이음 선배봇 CLI 테스트 모드에 오신 것을 환영합니다! 🌟")
    print("('종료'를 입력하면 테스트가 끝납니다.)\n")

    while True:
        user_input = input("질문: ").strip()
        
        if user_input in ["종료", "exit", "quit"]:
            print("\n선배봇: 다음에 또 궁금한 게 있으면 언제든 물어봐! 안녕~👋")
            break
        ...
        try:
            ...
            # 결과 출력
            print(f"\n[시스템 정보] 감지된 지역: {result['detected_region']}")
            print(f"[시스템 정보] 참조된 정책 수: {len(result['source_documents'])}개")
            print("-" * 50)
            print(f"\n{result['answer']}\n")
            print("-" * 50)
